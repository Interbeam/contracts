...
describe(' 0: Wormhole', () => {
  ...
  const { requestAirdrop, signAndPost, expectIxToSucceed, expectTxToSucceed } = boilerPlateReduction(connection, payer)

  const governance = new mock.GovernanceEmitter(GOVERNANCE_EMITTER_ADDRESS.toBuffer().toString('hex'), 20)
  ...
  describe('Verify Local Validator', function () {
    it('Create SPL Tokens', async function () {
      await Promise.all(
        Array.from(MINTS_WITH_DECIMALS.entries()).map(async ([mintDecimals, { privateKey, publicKey }]) => {
    ...
    it('Create ATAs', async function () {
      await Promise.all(
        Array.from(MINTS_WITH_DECIMALS.values()).flatMap(({ publicKey: mint }) =>
          [payer, relayer].map(
            (wallet) =>
              expect(getOrCreateAssociatedTokenAccount(connection, wallet, mint, wallet.publicKey)).to.be.fulfilled
          )
        )
      )
    })
    ...
    it("Mint to Wallet's ATAs", async function () {
      await Promise.all(
        Array.from(MINTS_WITH_DECIMALS.entries()).map(async ([mintDecimals, { publicKey: mint }]) => {
          ...
          await expect(mintTo(connection, payer, mint, destination, payer, mintAmount)).to.be.fulfilled
  ...
  describe('Verify Wormhole Program', function () {
    it('Initialize', async function () {
      ...
      const guardianSet = await wormhole.getGuardianSet(connection, CORE_BRIDGE_PID, info.guardianSetIndex)
  ...
  describe('Verify Token Bridge Program', function () {
    const ethereumTokenBridge = new mock.MockEthereumTokenBridge(WORMHOLE_CONTRACTS.ethereum.token_bridge)
    const tokenBridgeWethMint = tokenBridge.deriveWrappedMintKey(TOKEN_BRIDGE_PID, CHAINS.ethereum, ETH_WETH_ADDRESS)
    const tokenBridgeUsdcMint = tokenBridge.deriveWrappedMintKey(TOKEN_BRIDGE_PID, CHAINS.ethereum, ETH_USDC_ADDRESS)
    ...
    const registerForeignEndpoint = async (message: Buffer, isMalicious: boolean, expectedAccountLength: number) => {
      ...
      await expectIxToSucceed(createIxFunc(TOKEN_BRIDGE_PID, CORE_BRIDGE_PID, payer.publicKey, signedMsg))
    ...
    //   const { sequence } = await wormhole.getProgramSequenceTracker(connection, TOKEN_BRIDGE_PID, CORE_BRIDGE_PID)
    //   expect(sequence).equals(1n)
    // })

    it('Attest WETH from Ethereum', async function () {
      const signedMsg = await expect(
        signAndPost(ethereumTokenBridge.publishAttestMeta(ETH_WETH_ADDRESS, 18, 'WETH', 'Wrapped Ether'))
      ).to.be.fulfilled

      await expectTxToSucceed(
        createWrappedOnSolana(connection, CORE_BRIDGE_PID, TOKEN_BRIDGE_PID, payer.publicKey, signedMsg)
      )
    })

    it('Attest USDC from Ethereum', async function () {
      const signedMsg = await expect(
        signAndPost(ethereumTokenBridge.publishAttestMeta(ETH_USDC_ADDRESS, 6, 'USDC', 'USD Coin'))
      ).to.be.fulfilled
    ...
    it('Create WETH ATAs', async function () {
      await Promise.all(
        [payer, relayer].map(
          (wallet) =>
            expect(getOrCreateAssociatedTokenAccount(connection, wallet, tokenBridgeWethMint, wallet.publicKey)).to.be
              .fulfilled
        )
      )
    })
    ...
    it('Create USDC ATAs', async function () {
      await Promise.all(
        [payer, relayer].map(
          (wallet) =>
            expect(getOrCreateAssociatedTokenAccount(connection, wallet, tokenBridgeUsdcMint, wallet.publicKey)).to.be
              .fulfilled
        )
      )
    })
    ...
    it('Mint WETH to Wallet ATA', async function () {
      ...
      await expectTxToSucceed(redeemOnSolana(connection, CORE_BRIDGE_PID, TOKEN_BRIDGE_PID, payer.publicKey, signedMsg))
