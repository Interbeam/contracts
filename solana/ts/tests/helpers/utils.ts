...
export const boilerPlateReduction = (connection: Connection, defaultSigner: Signer) => {
  ...
  const requestAirdrop = async (account: PublicKey) =>
    connection.confirmTransaction(await connection.requestAirdrop(account, 1000 * LAMPORTS_PER_SOL))
  ...
  // sign with guardian and post the message to Solana
  const signAndPostMsg = async (message: Buffer, payer?: Signer) => {
    const signedMsg = guardianSign(message)
    await postSignedMsgAsVaaOnSolana(signedMsg, payer)
    return signedMsg
  }
