...
#[derive(Accounts)]
#[instruction(_params: RedeemNativeTransferWithPayloadParams)]
pub struct RedeemNativeTransferWithPayload<'info> {
    ...
    #[account(
        mut,
        constraint = payer.key() == recipient.key() || payer_token_account.key() == anchor_spl::associated_token::get_associated_token_address(&payer.key(), &mint.key()) @ InterbeamError::InvalidPayerAta
    )]
    ...
    #[account(
        address = config.token_bridge.custody_signer @ InterbeamError::InvalidTokenBridgeCustodySigner
    )]
...
pub struct RedeemNativeTransferWithPayloadParams {
    vaa_hash: [u8; 32],
}

pub fn redeem_native_transfer_with_payload(
    ctx: Context<RedeemNativeTransferWithPayload>,
    _params: &RedeemNativeTransferWithPayloadParams,
) -> Result<()> {
